...
function removeHitOverlay() {
	var findButton = className("android.widget.Button").text(
		"赠送主播礼物表示心意"
	);
...
function getPercent() {
	...
	if (result) {
		...
		var totalRob = parseInt(
			totalRobString.substring(0, totalRobString.indexOf("人"))
		);
		var totalAward = parseInt(
			totalAwardString.substring(0, totalAwardString.indexOf("个"))
		);
...
function backToGetMoney() {
	...
	for (var i = getMoneyTimeArray.length - 1; i >= 0; i--) {
		...
		if (
			(getMoneySeconds * 1000) / fastBackRate <
			Math.abs(getMoneyUpTimes)
		) {
			getMoneyTimeArray.splice(i, 1);
			getMoneyUpTimesArray.splice(i, 1);
		} else if (
			(Math.abs(getMoneyUpTimes) * fastBackRate) / 1000 >
			getMoneySeconds - backTimeWait
		) {
...
function clickGetMoneyButton(waitSeconds) {
	...
	for (var i = 0; i < waitSeconds; i++) {
		removeOverlay();
		var moneyControls = className("android.widget.Button").descStartsWith(
			"福袋"
		);
			...
			for (var i = 0; i < 5; i++) {
				...
				if (commonPublishControlFind.exists()) {
					...
					if (getPercent() > minPercent) {
						var commonPublishControl =
							commonPublishControlFind.findOne();
						...
						//等待开奖
						var needWaitTime =
							waitTillSeconds - new Date().getTime() + 5000;
						if (needWaitTime > 0) {
							console.log(
								"停留" + needWaitTime / 1000 + "秒等待抢福袋"
							);
							sleep(needWaitTime);
						}
					} else {
...
while (true) {
	...
	if (moneyControls.exists()) {
		...
		leftSeconds = parseInt(
			leftSecondString.substring(0, leftSecondString.indexOf("秒"))
		);
		...
		if (
			commonPublishControlFind.exists() &&
			!funsNecessary &&
			percentMeet &&
			leftTimeMeet
		) {
